...
public class CallManager {
	...
	public void init(Context context) {
		...
		// 设置音视频通话采样率，为了减少噪音，可以将采集了适当调低，这里默认设置32k
	...
	public void saveCallMessage() {
		L.d("The call ends and the call log message is saved! " + endType);
		...
		switch (endType) {
			case NORMAL: // 正常结束通话
				...
				break;
			case CANCEL: // 取消
				content = context.getString(R.string.call_cancel_local);
				break;
			case CANCELLED: // 被取消
				content = context.getString(R.string.call_cancel_oppo);
				break;
			case BUSY: // 对方忙碌
			...
			case OFFLINE: // 对方不在线
				...
				break;
			case REJECT: // 拒绝的
				content = context.getString(R.string.call_reject_local);
				break;
			case REJECTED: // 被拒绝的
				content = context.getString(R.string.call_reject_oppo);
				break;
			case NORESPONSE: // 未响应
				content = context.getString(R.string.call_no_response);
				break;
			case TRANSPORT: // 建立连接失败
				content = context.getString(R.string.call_connect_failed);
				break;
	...
	public void makeCall() {
		try {
			if (callType == CallType.VIDEO) {
				EMClient.getInstance().callManager().makeVideoCall(chatId, "ext数据");
			} else {
				EMClient.getInstance().callManager().makeVoiceCall(chatId, "ext数据");
			}
	...
	public void endCall() {
		try {
			...
			e.printStackTrace();
			L.e("结束通话失败:" + e.getErrorCode() + e.getMessage());
		}
		// 挂断电话调用保存消息方法
		saveCallMessage();
		// 通话结束，重置通话状态
...
//	}
	...
	private void loadSound() {
		if (isInComingCall) {
			loadId = soundPool.load(context, R.raw.sound_call_in, 1);
		} else {
			loadId = soundPool.load(context, R.raw.sound_call_out, 1);
		}
	}
		...
		if (isLoaded) {
			...
			soundPool.setOnLoadCompleteListener(new SoundPool.OnLoadCompleteListener() {
				...
				public void onLoadComplete(SoundPool soundPool, int i, int i1) {
					L.d("SoundPool load complete! loadId:" + loadId);
					isLoaded = true;
					// 首次监听到加载完毕，开始播放音频
					playCallSound();
				}
