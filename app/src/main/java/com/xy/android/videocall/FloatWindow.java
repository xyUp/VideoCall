...
public class FloatWindow {
	...
	public void addFloatWindow() {
		...
		// 获取要现实的布局
		floatView = LayoutInflater.from(context).inflate(R.layout.view_call_float_window, null);
		// 添加悬浮窗 View 到窗口
		windowManager.addView(floatView, layoutParams);
		if (CallManager.getInstance().getCallType() == CallManager.CallType.VOICE) {

			floatView.findViewById(R.id.llay_call_voice).setVisibility(View.VISIBLE);
			floatView.findViewById(R.id.flay_call_video).setVisibility(View.GONE);
			callTimeView = (TextView) floatView.findViewById(R.id.text_call_time);
			refreshCallTime();

		} else {

			floatView.findViewById(R.id.llay_call_voice).setVisibility(View.GONE);
			floatView.findViewById(R.id.flay_call_video).setVisibility(View.VISIBLE);

			localView = (EMCallSurfaceView) floatView.findViewById(R.id.em_local);
			oppositeView = (EMCallSurfaceView) floatView.findViewById(R.id.em_oppo);
		...
		// Button btnFloat = (Button) floatView.findViewById(R.id.btn_call_float);
			...
			public boolean onTouch(View v, MotionEvent event) {
				switch (event.getAction()) {
					case MotionEvent.ACTION_DOWN:
						...
						L.d("start x:" + startX + " y:" + startY);
						break;
					case MotionEvent.ACTION_MOVE:
						L.d("move x:" + event.getRawX() + " y:" + event.getRawY());
